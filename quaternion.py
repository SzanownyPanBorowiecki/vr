...
class Quaternion:
    ...
    def fromEuler(alpha, beta, gamma, rotationOrder="xyz", extrinsic=True):
        ...
        for axis in reversed(rotationOrder) if extrinsic else rotationOrder:
            ...
            if x:
                axisRotationMatrix = [[1, 0, 0], [
                    0, np.cos(alpha), -np.sin(alpha)], [0, np.sin(alpha), np.cos(alpha)]]
            if y:
                axisRotationMatrix = [[np.cos(beta), 0, np.sin(beta)], [
    ...
    def toEuler(self, rotationOrder="xyz", extrinsic=True):
        ...
        if rotationOrder.upper() not in ["ZXZ", "XYX", "YZY", "ZYZ", "XZX", "YXY", "XYZ", "YZX", "ZXY", "XZY", "ZYX", "YXZ"]:
            raise ValueError(
                "Invalid rotation order. Must be one of 'zxz', 'xyx', 'yzy', 'zyz', 'xzx', 'yxy', 'xyz', 'yzx', 'zxy', 'xzy', 'zyx', 'yxz'.")

        rotationOrder = rotationOrder.upper(
        ) if extrinsic else rotationOrder[::-1].upper()
        ...
        if noSingularities:
            ...
            if extrinsic:
                ...
                eulerAngles[0] = 2 * \
                    halfSum if not farEnoughFromZero else eulerAngles[0]
                eulerAngles[0] = 2 * \
                    halfDifference if not farEnoughFromPi else eulerAngles[0]
            else:
                eulerAngles[0] = 0 if not noSingularities else eulerAngles[0]
                eulerAngles[2] = 2 * \
                    halfSum if not farEnoughFromZero else eulerAngles[2]
                eulerAngles[2] = 2 * \
                    halfDifference if not farEnoughFromPi else eulerAngles[2]
    ...
    def inv(self):
        return self.conj() * (1.0/self.norm2())

    def norm2(self):
        return (self.w ** 2 + np.sum(self.V ** 2))
